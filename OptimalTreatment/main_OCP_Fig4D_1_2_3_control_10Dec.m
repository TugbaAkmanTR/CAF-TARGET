...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function main_OCP_Fig4D_1_2_3_control_10Dec()
...
while(test > toll)
    ...
    [Tx,X] = ode15s(@(t,x)stateEq_highE2(t,x,u1,u2,u3,Tu,E2), Tu_treatment(:,i), initx, options);

    % solve the adjoint eqn backward
    x1 = X(:,1); x2 = X(:,2); x3 = X(:,3); x4 = X(:,4);
    [Tlambda,Lambda] = ode15s(@(t,lambda) costateEqCD_highE2(t,lambda,u1,u2,u3,Tu_treatment(:,i),x1,x2,x3,x4,Tx,E2), ...
...
u1_high_dose_control_TypeI(:,i) = u1;

[Ju1,P1,P2]=cost_u1(TxCD,MCF7_with_CAF2_high_dose_control_TypeI,u1,u1);
fprintf('Type I \n');
...
u2_high_dose_control_TypeII(:,i) = u2;

[Ju2,P1,P2]=cost_u2(TxCD,MCF7_with_CAF2_high_dose_control_TypeII,u2,u2);
fprintf('Type II \n');
fprintf('Tumor size = %.3f, Control = %.3f, J = %.3f \n', P1,P2,Ju2);
...
u3_high_dose_control_TypeIII(:,i) = u3;

[Ju3,P1,P2]=cost_u3(TxCD,MCF7_with_CAF2_high_dose_control_TypeIII,u3,u3);
fprintf('Type III \n');
fprintf('Tumor size = %.3f, Control = %.3f, J = %.3f \n', P1,P2,Ju3);
...
u1_high_dose_control_TypeI_III(:,i) = u1;
u3_high_dose_control_TypeI_III(:,i) = u3;

[Ju1u3,P1,P2]=cost_u1_u3(TxCD,MCF7_with_CAF2_high_dose_control_TypeI_III,u1,u3);
fprintf('Type I+III \n');
fprintf('Tumor size = %.3f, Control = %.3f, J = %.3f \n', P1,P2,Ju1u3);
...
u2_high_dose_control_TypeII_III(:,i) = u2;
u3_high_dose_control_TypeII_III(:,i) = u3;

[Ju2u3,P1,P2]=cost_u2_u3(TxCD,MCF7_with_CAF2_high_dose_control_TypeII_III,u2,u3);
fprintf('Type II+III \n');
fprintf('Tumor size = %.3f, Control = %.3f, J = %.3f \n', P1,P2,Ju2u3);
...
figure(930)
subplot(3,2,1)
h(1)=plot(Tu_treatment(:,i),u1_high_dose_control_TypeI_III(:,i),'k-o','MarkerIndices', 1:50:length(Tu), 'MarkerSize', 6, 'LineWidth',2);
hold on
h(2)=plot(Tu_treatment(:,i),u3_high_dose_control_TypeI_III(:,i),'r-','MarkerIndices', 1:50:length(Tu), 'MarkerSize', 6, 'LineWidth',2);
grid on
...
xlim([24,tf])
ylim([0,1])
sgtitle('High dose of E2 - Control functions - u_1(t) (circle), u_2(t) (diamond) , u_3(t) (square)');

subplot(3,2,2)
h(3)=plot(Tu_treatment(:,i),u2_high_dose_control_TypeII_III(:,i),'b-d','MarkerIndices', 1:50:length(Tu), 'MarkerSize', 6, 'LineWidth',2);
hold on
h(4)=plot(Tu_treatment(:,i),u3_high_dose_control_TypeII_III(:,i),'r-','MarkerIndices', 1:50:length(Tu), 'MarkerSize', 6, 'LineWidth',2);
grid on
...
subplot(3,2,3)
h(5)=plot(Tu_treatment(:,i),u1_high_dose_control_TypeI(:,i),'k-o','MarkerIndices', 1:50:length(Tu), 'MarkerSize', 6, 'LineWidth',2);
grid on
xlabel('t')
ylabel('u_1(t)')
xlim([24,tf])
...
subplot(3,2,4)
h(6)=plot(Tu_treatment(:,i),u2_high_dose_control_TypeII(:,i),'b-d','MarkerIndices', 1:50:length(Tu), 'MarkerSize', 6, 'LineWidth',2);
grid on
xlabel('t')
ylabel('u_2(t)')
xlim([24,tf])
...
subplot(3,2,5)
h(7)=plot(Tu_treatment(:,i),u3_high_dose_control_TypeIII(:,i),'r-','MarkerIndices', 1:50:length(Tu), 'MarkerSize', 6, 'LineWidth',2);
grid on
xlabel('t')
ylabel('u_3(t)')
xlim([24,tf])
ylim([0,1])
sgtitle('Control functions - High dose E2 (0.5mg) (with CAF)','fontsize',18)
...
sgtitle('Optimal treatment - High dose E2 (0.5mg)','fontsize',18)
